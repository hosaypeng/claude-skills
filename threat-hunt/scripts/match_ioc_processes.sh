...
for pattern in "ioc_pegasus_" "ioc_candiru_"; do
  ...
  grep "^process|" "$ioc_file" 2>/dev/null | while read -r line; do
    proc_name=$(echo "$line" | cut -d'|' -f2)
    desc=$(echo "$line" | cut -d'|' -f3)
    [ -z "$proc_name" ] && continue

    # Check against running process names
    if echo "$running_procs" | grep -qx "$proc_name"; then
      echo "  [CRITICAL] Spyware process detected: $proc_name ($desc)"
      echo "    PIDs: $(ps -eo pid,comm= 2>/dev/null | grep "$proc_name" | awk '{print $1}' | tr '\n' ' ')"
      match_found=1
    fi

    # Check against process arguments
    if echo "$running_args" | grep -q "$proc_name"; then
      matching=$(echo "$running_args" | grep "$proc_name" | head -3)
